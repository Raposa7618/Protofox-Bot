    ...
    if reacts_enabled:
        ...
        for palavra, emoji in palavras_chave.items():
            if palavra in message.content.lower():
                ...
                await asyncio.sleep(0.5)  # Adiciona um delay de 0.5 segundos entre reações
...
async def tocar(ctx, url: str = None):
    if url is None:
        ...
        if not music_queue:
            await ctx.send("A fila está vazia. Adicione músicas com `!tocar <URL>` ou `!fila`.")
            return
        next_data = music_queue.popleft()
...
async def on_ready():
    ...
    # avisa sobre novo commit
    aviso = bot.get_channel(1400651787836330106)
    agora_brasilia = datetime.now(ZoneInfo("America/Sao_Paulo"))
    await aviso.send(f'Commit realizado em: {agora_brasilia.strftime("%d/%m/%Y %H:%M:%S")}')
